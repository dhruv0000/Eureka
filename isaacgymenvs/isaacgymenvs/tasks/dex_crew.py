...
from isaacgymenvs.tasks.dexscrew_utils import sample_cylinder, tprint
from isaacgymenvs.tasks.base.dexscrew_vec_task import VecTask
import torch.nn.functional as F


class DexCrewBase(VecTask):
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture=False, force_render=False):
        self.config = cfg
        self.asset_root = os.path.normpath(os.path.join(os.path.dirname(os.path.abspath(__file__)), "../../"))
        # before calling init in VecTask, need to do
        # 0. setup the dim info
        self.numActions = cfg['env']['numActions']
        self.fingers_num = 5
        # 1. setup randomization
        self._setup_domain_rand_config(cfg['env']['randomization'])
        # 2. setup privileged information
        self._setup_priv_option_config(cfg['env']['privInfo'])
        # 3. setup object assets
        self._setup_object_info(cfg['env']['object'])
        # 4. setup rewards
        self._setup_reward_config(cfg['env']['reward'])
        # unclassified config
        self.base_obj_scale = cfg['env']['baseObjScale']
        self.aggregate_mode = self.config['env']['aggregateMode']
        self.up_axis = 'z'
        self.rotation_axis = cfg['env']['rotation_axis']
        self.reset_z_threshold = self.config['env']['reset_z_threshold']
        self.reset_dist_threshold = self.config['env']['reset_dist_threshold']
        self.with_camera = cfg['env']['enableCameraSensors']
        self.nut_termination_history_len = cfg['env']['object']['nut_termination_history_len'] 
        self.nut_stagnation_eps = cfg['env']['object']['nut_stagnation_eps']
        self.screw_upper_limit = cfg['env']['object']['screw_upper_limit']
    ...
    def compute_reward(self, actions):
        ...
        orig_reward = compute_hand_reward(
        ...
        if 'gpt_reward' not in self.extras:
            self.rew_buf[:] = orig_reward
        else:
            self.extras['orig_reward'] = orig_reward.mean()
    ...
    def _setup_object_info(self, o_config):
        ...
        for p_id, prim in enumerate(primitive_list):
            if 'screw' in prim:
                ...
                screw_compounds = sorted(glob(os.path.join(self.asset_root, 'assets/screw', subset_name, '*.urdf')))
                ...
                for i, name in enumerate(screw_compounds):
                    self.asset_files_dict[f'screw_{i}'] = os.path.relpath(name, self.asset_root)
...
class DexCrewScrewDriver(DexCrewBase):
    pass


class DexCrewNutBolt(DexCrewBase):
    pass
